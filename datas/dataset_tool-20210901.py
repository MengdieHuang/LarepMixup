...
def open_stl10_128(tarball: str, *, max_images: Optional[int]):
    ...
    images = images.reshape(-1, 3, 96, 96)                                                              #   stl10原是96x96像素，但为了stylegan2ada使用，需改成2的幂次方                                               
...
def open_stl10_64(tarball: str, *, max_images: Optional[int]):
    ...
    def iterate_images():
        # for idx, img in enumerate(images):
        for idx, (img, lab) in enumerate(train_dataloader):
            # print("image index:",idx)               
            # print("img.shape",img.shape)            #   img.shape torch.Size([1, 3, 64, 64])
            img.reshape(-1, 3, 64, 64)
            # print("img.shape",img.shape)        
            img = img.numpy()
            # print("img.shape",img.shape)            #   img.shape (1, 3, 64, 64)
            img =img[-1]
            # print("img.shape",img.shape)            #   img.shape (3, 64, 64)   
            
            lab = lab.numpy()
            # print("img.type:",type(img))
            # print("lab.type:",type(lab))
            # print("img.shape:",img.shape)           #   img.shape: (3, 64, 64)        
            # print("lab:",lab)                       #   img.dtype uint8
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == 'cifar-10-python.tar.gz':
            ...
            # return open_stl10_128(source, max_images=max_images)
            return open_stl10_64(source, max_images=max_images)

        elif os.path.basename(source) == 'ILSVRC2012_img_train.tar':
            return open_imagenetmixed10(source, max_images=max_images)
