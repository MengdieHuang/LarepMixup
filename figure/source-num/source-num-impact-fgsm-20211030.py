...
# 读取数据
with open("cifar10-preactresnet18-whitebox-fgsm-acc-convex-20211028.csv",'r') as f:
...
with open("cifar10-preactresnet34-whitebox-fgsm-acc-convex-20211029.csv",'r') as f:
    data_csv = csv.reader(f)

    step = []
    preactresnet34_convex_dual = []
    preactresnet34_convex_ternary = []    

    next(data_csv)
    for row in data_csv:
        step.append(str(int(row[0])))
        preactresnet34_convex_dual.append(float(row[1]))
        preactresnet34_convex_ternary.append(float(row[2]))

with open("cifar10-preactresnet34-whitebox-fgsm-acc-mask-20211030.csv",'r') as f:
    data_csv = csv.reader(f)

    step = []
    preactresnet34_mask_dual = []
    preactresnet34_mask_ternary = []    

    next(data_csv)
    for row in data_csv:
        step.append(str(int(row[0])))
        preactresnet34_mask_dual.append(float(row[1]))
        preactresnet34_mask_ternary.append(float(row[2]))
...
plt.plot(step,preactresnet34_convex_dual, label=f'dual mixup', marker='s', markersize=3)
plt.plot(step,preactresnet34_convex_ternary, label=f'ternary mixup', marker='D', markersize=3)
...
plt.plot(step,preactresnet34_mask_dual, label=f'dual mixup', marker='s', markersize=3)
plt.plot(step,preactresnet34_mask_ternary, label=f'ternary mixup', marker='D', markersize=3)
...
savename = f'cifar10-preactresnet18-preactresnet34-whitebox-fgsm-acc-convex-mask-20211030'
