...
with open("cifar10-googlenet-cle-adv-acc-black-fgsm.csv",'r') as f:
...
# ax1.bar(x - 5*(width/2), std_normal, width, color= 'dodgerblue', label=f'Standard trained AlexNet on normal samples' )
# ax1.bar(x - 3*(width/2), repmix_p5_normal, width, color= 'deepskyblue', label=f'RepMixup({beta_str}={0.5:.1f}) trained AlexNet on normal samples ' )
# ax1.bar(x - 1*(width/2), repmix_2_normal, width, color= 'lightskyblue', label=f'RepMixup({beta_str}={2:.1f}) trained AlexNet on normal samples ' )
...
# ax2.bar(x + 1*(width/2), std_fgsm, width, color= 'orangered', label=f'Standard trained AlexNet on FGSM samples')
# ax2.bar(x + 3*(width/2), repmix_p5_fgsm,width, color= 'darkorange', label=f'RepMixup({beta_str}={0.5:.1f}) trained AlexNet on FGSM samples')
# ax2.bar(x + 5*(width/2), repmix_2_fgsm,width, color= 'gold', label=f'RepMixup({beta_str}={2:.1f}) trained AlexNet on FGSM samples')
...
#---------标题---------
# plt.title(f'Accuracy of GoogleNet on Black-box Pixel-level Adversarial Attacks',fontsize=12, pad=12)
plt.title(f'GoogleNet against Black-box Pixel-level Adversarial Attacks',fontsize=12, pad=12)

plt.show()
savepath = f'/home/maggie/mmat/figure/black-fgsm'
savename = f'cifar10-googlenet-cle-adv-acc-black-fgsm-20211104'
