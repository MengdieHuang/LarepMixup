...
#-----------------模型处理---------------------
    ...
    cle_x_train, cle_y_train = target_classifier.dataarray(cle_train_dataloader)
    cle_x_test, cle_y_test = target_classifier.dataarray(cle_test_dataloader)
    ...
    cle_test_accuracy, cle_test_loss = target_classifier.evaluate(target_classifier.model(),cle_test_dataloader)
    ...
    cle_test_accuracy_2, cle_test_loss_2 = target_classifier.EvaluateClassifier(target_classifier.model(),cle_x_test,cle_y_test)                    #   测试一下evaluate和EvaluateClassifier计算结果是否一致
    print(f'standard trained classifier *accuary2* on clean testset:{cle_test_accuracy_2 * 100:.4f}%' )                                             #   *accuary2* on clean testset:71.2100%
    print(f'standard trained classifier *loss2* on clean testset:{cle_test_loss_2}' ) 
...
#-----------------对抗攻击---------------------
    ...
    adv_test_accuracy, adv_test_loss = target_classifier.EvaluateClassifier(target_classifier.model(),x_test_adv,y_test_adv)
...
#         SaveTxt(args,exp_result_dir,cle_test_accuracy,adv_test_accuracy,at_adv_test_accuracy,cle_test_loss,adv_test_loss,at_adv_test_loss)
...
#-----------------混合训练防御---------------------
    if args.defense_mode == "mmat":
        ...
        print("x_train_mix.type: ", type(x_train_mix))                                                              #   x_train_mix.type:  <class 'numpy.ndarray'>
        print("x_train_mix.shape: ", x_train_mix.shape)                                                             #   x_train_mix.shape:  (9, 3, 32, 32)
        print("y_train_mix.type: ", type(y_train_mix))                                                              #   y_train_mix.type:  <class 'numpy.ndarray'>
        print("y_train_mix.shape: ", y_train_mix.shape)                                                             #   y_train_mix.shape:  (9, 10)

        
        #   原始样本标签转one hot
        print("cle_x_train.type: ", type(cle_x_train))                                                              #   numpy.ndarray
        print("cle_x_train.shape: ", cle_x_train.shape)                                                             #   cle_x_train.shape:  (50000, 3, 32, 32)
        print("cle_y_train.type: ", type(cle_y_train))                                                              #   cle_y_train.type:  <class 'list'>
        device = torch.device('cuda')
        cle_y_train = torch.tensor(cle_y_train).to(device)
        cle_y_train_onehot = torch.nn.functional.one_hot(cle_y_train, args.n_classes).float().to(device)      
        print("cle_y_train_onehot.shape: ", cle_y_train_onehot.shape)           
        cle_y_train_onehot = cle_y_train_onehot.cpu().numpy()
        print("cle_y_train_onehot.type: ", type(cle_y_train_onehot))                                                #   cle_y_train_onehot.type:  <class 'numpy.ndarray'>
        print("cle_y_train_onehot.shape: ", cle_y_train_onehot.shape)                                               #   cle_y_train_onehot.shape:  (50000, 10)


        #   扩增混合样本
        aug_x_train = np.append(cle_x_train, x_train_mix, axis=0)
        print("aug_x_train.type: ", type(aug_x_train))                                                              #   numpy.ndarray
        print("aug_x_train.shape: ", aug_x_train.shape)                                                             #   aug_x_train.shape:  (50009, 3, 32, 32)
        aug_y_train = np.append(cle_y_train_onehot, y_train_mix, axis=0)
        print("aug_y_train.type: ", type(aug_y_train))                                                              #   numpy.ndarray
        print("aug_y_train.shape: ", aug_y_train.shape)                                                             #   (50009,10)


        #   对抗攻击原始分类器
        # adv_test_accuracy_2, adv_test_loss_2 = attack_classifier.EvaluateClassifier(target_classifier.model(),x_test_adv,y_test_adv)
        adv_test_accuracy_2, adv_test_loss_2 = target_classifier.EvaluateClassifier(target_classifier.model(),x_test_adv,y_test_adv)
        print(f'standard trained classifier accuary2 on adversarial testset:{adv_test_accuracy_2 * 100:.4f}%' ) 
        print(f'standard trained classifier loss2 on adversarial testset:{adv_test_loss_2}' )   


        #   混合训练（MMAT）
        target_classifier.artmodel().fit(aug_x_train, aug_y_train, x_test_adv, y_test_adv, exp_result_dir, args, nb_epochs=args.epochs, batch_size=args.batch_size)
        # target_classifier.artmodel().fit(cle_x_train, cle_y_train, x_test_adv, y_test_adv, exp_result_dir, args, nb_epochs=args.epochs, batch_size=args.batch_size)
        # target_classifier.artmodel().fit(x_train_mix, y_train_mix, x_test_adv, y_test_adv, exp_result_dir, args, nb_epochs=args.epochs, batch_size=args.batch_size)
        
        mmat_adv_test_accuracy, mmat_adv_test_loss = target_classifier.EvaluateClassifier(target_classifier.model(),x_test_adv,y_test_adv)
        ...
        mmat_cle_test_accuracy, mmat_cle_test_loss = target_classifier.evaluate(target_classifier.model(),cle_test_dataloader)
