...
#-----------------混合训练防御---------------------
    if args.defense_mode == "mmat":
        ...
        #   混合样本生成
        x_train_mix, y_train_mix = generate_model.generatedtrain()
        print("x_train_mix.type: ", type(x_train_mix))                          #   numpy.ndarray
        print("x_train_mix.shape: ", x_train_mix.shape)                         #   (9, 3, 32, 32)
        print("y_train_mix.type: ", type(y_train_mix))                          #   numpy.ndarray
        print("y_train_mix.shape: ", y_train_mix.shape)                         #   (9,8,10)    #为什么会有一个8


        print("cle_x_train.type: ", type(cle_x_train))                          #   numpy.ndarray
        print("cle_x_train.shape: ", cle_x_train.shape)                         #   (50000, 3, 32, 32)
        
        #   原始样本标签转one hot
        print("cle_y_train.type: ", type(cle_y_train))                          #   list
        device = torch.device('cuda')
        cle_y_train = torch.tensor(cle_y_train).to(device)
        # print("cle_y_train.type: ", type(cle_y_train))                          #   torch tensor gpu tensor
        # print("cle_y_train.shape: ", cle_y_train.shape)                         #   torch.Size([50000])
        cle_y_train_onehot = torch.nn.functional.one_hot(cle_y_train, args.n_classes).float().to(device)      #   y成了gpu tensor
        print("cle_y_train_onehot.shape: ", cle_y_train_onehot.shape)           #   torch.Size([50000,10])
        
        cle_y_train_onehot = cle_y_train_onehot.cpu().numpy()
        print("cle_y_train_onehot.type: ", type(cle_y_train_onehot))            #   numpy.ndarray
        print("cle_y_train_onehot.shape: ", cle_y_train_onehot.shape)           #   (50000,10)
        ...
        print("aug_x_train.shape: ", aug_x_train.shape)         #   (50009, 32, 32, 3)

        aug_y_train = np.append(cle_y_train_onehot, x_train_mix, axis=0)
        ...
        SaveTxt(args,exp_result_dir,cle_test_accuracy,adv_test_accuracy,mmat_adv_test_accuracy,cle_test_loss,adv_test_loss,mmat_adv_test_loss)
