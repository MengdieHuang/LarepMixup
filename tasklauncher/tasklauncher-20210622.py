...
#-----------------混合训练防御---------------------
        ...
        generate_model = MixGenerate(args)
        generate_model.train(args)
        generate_model.generate(args)
        
        #   混合样本生成
        x_train_mix, y_train_mix = generate_model.generate(args)

        #   扩增混合样本
        aug_x_train = np.append(cle_x_train, x_train_mix, axis=0)
        aug_y_train = np.append(cle_y_train, y_train_mix, axis=0)
