...
if __name__ == '__main__':
    ...
    # launchering task
    # target_classifier = tasks.clatrain.ClaTrain(args,exp_result_dir)    # target_classifier是torchivision.models类型
    # attack_classifier,aug_train_dataloader = tasks.claattack.ClaAttack(args,exp_result_dir,target_classifier)
    # adv_trained_target_classifier = tasks.cladefense.ClaAdvTrain(target_classifier,aug_train_dataloader)

    print("Flag: standard training classifier !")
    target_classifier = Classifier(args)
    target_classifier.train(exp_result_dir) 
    cle_test_accuracy, cle_test_loss = target_classifier.evaluate(target_classifier.classify_model,target_classifier.test_dataloader,exp_result_dir)
    ...
    attack_classifier = AdvAttackClassifier(args,target_classifier.classify_model)
    ...
    aug_train_dataloader, adv_test_dataloader = attack_classifier.generate(exp_result_dir)   #   (100000,32.32.3)
    adv_test_accuracy, adv_test_loss = attack_classifier.evaluate(target_classifier.classify_model,adv_test_dataloader,exp_result_dir)
    ...
    # if args.pretrained_on_imagenet == True:
    #     loss_txt=open(f'{exp_result_dir}/pretrained-classifier-{args.model}-loss-on-clean-{args.dataset}-testset.txt', "w")    
    #     loss_txt_content = f'{exp_result_dir}/pretrained-classifier-{args.model}-loss-on-clean-imagenet-testset = {cle_test_loss}'
    #     loss_txt.write(str(loss_txt_content))
    # elif args.pretrained_on_imagenet == False:
    #     loss_txt=open(f'{exp_result_dir}/standard-trained-classifier-{args.model}-loss-on-clean-{args.dataset}-testset.txt', "w")    
    #     loss_txt_content = f'{exp_result_dir}/standard-trained-classifier-{args.model}-loss-on-clean-{args.dataset}-testset = {cle_test_loss}'
    #     loss_txt.write(str(loss_txt_content))


    # if args.pretrained_on_imagenet == True:
    #     accuracy_txt=open(f'{exp_result_dir}/pretrained-classifier-{args.model}-accuracy-on-adv-{args.dataset}-testset.txt', "w")    
    #     txt_content = f'{exp_result_dir}/pretrained-classifier-{args.model}-accuracy-on-on-adv-{args.dataset}-testset = {adv_test_accuracy}'
    #     accuracy_txt.write(str(txt_content))
    # elif args.pretrained_on_imagenet == False:
    #     accuracy_txt=open(f'{exp_result_dir}/standard-trained-classifier-{args.model}-accuracy-on-adv-{args.dataset}-testset.txt', "w")    
    #     txt_content = f'{exp_result_dir}/standard-trained-classifier-{args.model}-accuracy-on-adv-{args.dataset}-testset = {adv_test_accuracy}'
    #     accuracy_txt.write(str(txt_content))

    # if args.pretrained_on_imagenet == True:
    #     loss_txt=open(f'{exp_result_dir}/pretrained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset.txt', "w")    
    #     loss_txt_content = f'{exp_result_dir}/pretrained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset = {adv_test_loss}'
    #     loss_txt.write(str(loss_txt_content))
    # elif args.pretrained_on_imagenet == False:
    #     loss_txt=open(f'{exp_result_dir}/standard-trained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset.txt', "w")    
    #     loss_txt_content = f'{exp_result_dir}/standard-trained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset = {adv_test_loss}'
    #     loss_txt.write(str(loss_txt_content))

    # accuracy_txt=open(f'{exp_result_dir}/adversarial-trained-classifier-{args.model}-accuracy-on-adv-{args.dataset}-testset.txt', "w")    
    # txt_content = f'{exp_result_dir}/adversarial-trained-classifier-{args.model}-accuracy-on-adv-{args.dataset}-testset = {at_adv_test_accuracy}'
    # accuracy_txt.write(str(txt_content))
    # loss_txt=open(f'{exp_result_dir}/adversarial-trained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset.txt', "w")    
    # loss_txt_content = f'{exp_result_dir}/adversarial-trained-classifier-{args.model}-loss-on-adv-{args.dataset}-testset = {at_adv_test_loss}'
    # loss_txt.write(str(loss_txt_content))